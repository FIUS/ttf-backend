...
module.exports = env => {
  ...
  const postcssPlugins = function() {
    // safe settings based on:
    // https://github.com/ben-eb/cssnano/issues/358#issuecomment-283696193
    ...
    return [
      postcssUrl({
        url: (URL) => {
          ...
          if (deployUrl.match(/:\/\//)) {
            // If deployUrl contains a scheme, ignore baseHref use deployUrl as
            // is.
  ...
  return {
        ...
        },
        {
          ...
          'use': [
            ...
            },
            {
              'loader': 'sass-loader',
              'options':
                  {'sourceMap': false, 'precision': 8, 'includePaths': []}
            }
          ]
        },
        {
    ...
    'plugins': [
      new NoEmitOnErrorsPlugin(), new GlobCopyWebpackPlugin({
        'patterns': ['assets', 'favicon.ico'],
        'globOptions': {'cwd': './src', 'dot': true, 'ignore': '**/.gitkeep'}
      }),
      new ProgressPlugin(), new HtmlWebpackPlugin({
      ...
      new NamedModulesPlugin({}), new AotPlugin({
        'mainPath': 'main.ts',
        'hostReplacementPaths': {
          'environments/environment.ts': (env != null && env.production) ?
      ...
      new ManifestRevisionPlugin(
          ...
          {rootAssetPath: '.src', ignorePaths: ['/styles', '/scripts']})
    ],
    'node': {
      'fs': 'empty',
      'global': true,
